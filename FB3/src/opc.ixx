...
struct PackagePart {
    ...
    wxl::core::u8_text contentType;  ///< "application/fb3-body+xml" и т.п.
};
...
public:
    ...
    /// Тот же пакет, но из уже прочитанных байтов.
    ///
    /// Ради этого он и есть: файл читает рабочий поток, а разбирает
    /// интерфейсный, и открывать файл во второй раз -- здесь и синхронно --
    /// значило бы вернуть то, от чего уходили.
    ///
    /// Байтами пакет не владеет и читает части по требованию, так что они
    /// обязаны жить дольше него. `Document` держит их у себя.
    ///
    /// @throw std::runtime_error, если это не пакет OPC.
    explicit OpcPackage(std::span<const std::byte> bytes);
    ~OpcPackage();
    ...
    std::vector<std::pair<wxl::core::u8_text, PackagePart>> relationshipsOfType(
