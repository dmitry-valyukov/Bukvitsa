// Те два разбора значений атрибутов, которым нужен wxl::core.
//
...
std::optional<int> toInt(const std::string_view value) {
    ...
    int number = 0;
    if (!wxl::core::try_parse(value, number)) return std::nullopt;
    return number;
}

Length toLength(const std::optional<wxl::core::u8_view> value) {
    Length length;
    ...
    const auto parsed = wxl::core::parse_prefix<float>(value->chars());
