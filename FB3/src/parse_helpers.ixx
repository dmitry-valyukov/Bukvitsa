...
/// То же для куска чужого текста — например, первых четырёх байт ISO-даты.
/// Отдельная перегрузка потому, что число — это цифры, а не текст: правильность
/// UTF-8 разбору не нужна и не проверяется. Иначе кусок пришлось бы прогонять
/// через checked() только затем, чтобы собрать тип, которым здесь не пользуются.
std::optional<int> toInt(std::string_view value);
