...
//
// Панель — часть обстановки, и красится она обстановкой (chrome.h), которая
// идёт за темой бумаги: светлые ящики над дневной страницей, тёмные над
// ночной. Кисти привязаны к её полям, и смена темы перекрашивает ящики без
// участия панели; сама она переставляет только отметки вкладки и темы.
...
public:
    ...
    /// @param chrome обстановка, которой красятся ящики.
    ReaderPanel(const wxl::Compositor& compositor, BookView& view, Chrome& chrome);

    /// Обстановка перекрасилась: отметки вкладки и темы ставятся заново. Они
    /// не привязаны к обстановке, потому что переключаются сами.
    void remark();
